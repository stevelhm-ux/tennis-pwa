...
/** Find a player by exact name in a workspace; returns id or null */
...
/** Ensure a player exists (by name) in a workspace; returns id */
...
/** List matches (optionally filtered by tournament) within a workspace. */
...
/** Generic partial update; returns the updated row. */
...
/**
 * Update the opponent by **name**:
 * - ensure (or create) a Player row in the same workspace,
 * - set matches.player_b_id to that player's id,
 * - return the updated match.
 */
export async function updateMatchOpponent(params: {
  matchId: string
  wsId: string
  opponentName: string
}): Promise<Match> {
  ...
  if (!name) throw new Error('opponentName is required')

  const playerBId = await ensurePlayerId(wsId, name)
